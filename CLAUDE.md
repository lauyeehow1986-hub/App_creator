...
| `build_portable()` native runtimes | `native_runtime_providers()` registry stages the *external* runtime a package needs (impossible in wasm, hence portable-only): **rJava** (portable Temurin JRE) and **tesseract** (OCR traineddata) are **verified end-to-end offline on Windows**; **RMariaDB** client works offline as-is; the opt-in `RMariaDB(server=TRUE)` (portable MariaDB server) and `rstan`/`brms`(`rtools=TRUE`) (bundled Rtools) paths are **scaffolded but NOT yet run** — each warns loudly at build time. See `docs/ARCHITECTURE.md` "Native-runtime provisioning". |
